...
(function ($, window, undefined) {
	...
	function mightySlider(frame, options, callbackMap) {
		...
		function load(immediate) {
			...
			if (navigationType) {
				...
				$slides.each(function (i, element) {
					...
					$('.' + minnamespace + 'Caption', $slide).each(function (index, caption) {
						...
						// Set isParallax value to slideData if any captions has parallaxLevel options
						if (captionOptions.parallaxLevel) {
							slideData.isParallax = 1;
						}

						// Set hasCaptionMediaEnabled value to slideData if any captions has media options
						if (captionOptions.cover || captionOptions.video || captionOptions.source) {
							slideData.hasCaptionMediaEnabled = 1;
						}
			...
			if (frameSize > 0) {
				...
				if ($pagesBar[0]) {
					...
					$pages.eq(self.relative.activePage).addClass(self.options.classes.activeClass);
				}
			}
		...
		function activate(slide) {
			var index = getIndex(slide),
				lastActive = self.relative.activeSlide;
			...
			if (last.active !== index) {
				var slideData = self.slides[index];
				...
				if (self.slides[self.relative.activeSlide] && self.slides[self.relative.activeSlide].isParallax) {
					$parent.off('.' + uniqId);
					...
					if (!isTouch) {
						...
						$parent.on(mouseEnterEvent, function (event) {
							...
							var offset = $parent.offset(),
								startX = event.isTrigger ? 0 : event.pageX - offset.left,
								startY = event.isTrigger ? 0 : event.pageY - offset.top;

							// Add :mousemove event to the $parent
							$parent.off(mouseLeaveEvent).on(mouseMoveEvent, function (e) {
								// Calculate the X & Y differences from started dimensions
								parallax.X = (e.pageX - offset.left) - startX,
								parallax.Y = (e.pageY - offset.top) - startY;
								...
								if (self.options.parallax.revert) {
									revertParallax();
								}
							});
					...
					self.one('slideLoaded', function(event, slideIndex) {
						if (slideIndex === index) {
							captionID = setTimeout(function () {
								...
								clearTimeout(captionID);
							}, lastActive < 0 ? 0 : self.options.speed);
						}
					});
				}
		...
		self.resume = function (priority) {
			...
			}, self.slides[self.relative.activeSlide].options.pauseTime || self.options.cycling.pauseTime);
		};
		...
		function dragInit(event) {
			...
			dragging.initPage = self.relative.activePage;
		...
		function dragHandler(event) {
			...
			slideTo(dragging.tweese && self.options.dragging.onePage ? self.pages[within(dragging.delta < 0 ? dragging.initPage + 1 : dragging.initPage - 1, 0, self.pages.length - 1)] : Math.round(dragging.initPos - dragging.delta));
		}
		...
		function loadSlide(slide) {
			...
			if (slideData.hasCaptionMediaEnabled) {
				...
				for (var i = 0, len = mediaCaptions.length; i < len; i++) {
					var caption = mediaCaptions[i],
		...
		function setCover(obj) {
			...
			if (obj.cover) {
				...
				if (obj.isVideo) {
					...
					trigger('coverInserted', obj.index);
					
					$videoEl.one('loadedmetadata', function() {
						var videoEl = $videoEl[0];

						// Store cover natural width and height
						$videoEl.data({ naturalWidth: videoEl.videoWidth, naturalHeight: videoEl.videoHeight }).show();
						...
						// Unbind the :progress event for video element
						$videoEl.off('progress.' + namespace);
		...
		function setSlidesCovers() {
			...
			function eachHandler(slide, i) {
				...
				var coverURL = cover || parsePhotoURL(slide.options.video) && slide.options.video,
					...
					console.log('lastCover:', lastCover);
					console.log('coverUN:', coverUN);
		...
		function repositionCovers(slide) {
			...
			for (var i = 0, len = newSlides.length; i < len; i++) {
		...
		function clearCaptions(slide) {
			...
			if (self.slides[index] && self.slides[index].captions.length) {
				...
				self.one('moveEnd', function() {
					$captions.removeAttr('style').removeClass(self.options.classes.showedCaptionsClass);
				});
		...
		function parallaxCaptions() {
			...
			for(var i = 0, len = parallax.parallaxCaptions.length; i < len; i++) {
				var caption = parallax.parallaxCaptions[i],
					...
					// Alowed parallax effect dimensions
					parallaxDimensions = caption.options.parallaxDimensions || self.options.parallax;

				// Normalize parallax original values
				parallax.originals[i] = {};

				// Calculate X parallax dimension
				if (parallaxDimensions.x) {
					parallax.originals[i].x = parseInt((self.options.parallax.invertX ? -parallax.X : parallax.X) / 100 * level);

					// Set style to caption element
					captionEl.style['marginLeft'] = parallax.originals[i].x + 'px';
				}

				// Calculate Y parallax dimension
				if (parallaxDimensions.y) {
					parallax.originals[i].y = parseInt((self.options.parallax.invertY ? -parallax.Y : parallax.Y) / 100 * level);

					// Set style to caption element
					captionEl.style['marginTop'] = parallax.originals[i].y + 'px';
				}

				// Calculate 3D parallax dimensions
				if (parallaxDimensions.z && gpuAcceleration) {
					parallax.originals[i].x = parseInt((self.options.parallax.invertX ? -parallax.X : parallax.X) / 100 * level),
					parallax.originals[i].y = parseInt((self.options.parallax.invertY ? -parallax.Y : parallax.Y) / 100 * level),
		...
		function revertParallax() {
			var timeOut = self.options.parallax.revertDuration;

			function moveCaptions(timestamp) {
				if(!captionParallax.start) captionParallax.start = captionParallax.lastTime = timestamp;
				captionParallax.now = timestamp;

				var progress = captionParallax.now - captionParallax.start,
				...
				for(var i = 0, len = parallax.parallaxCaptions.length; i < len; i++) {
					...
					// Calculate X parallax dimension to original value
					if (parallaxDimensions.x) {
						var x = (parallax.originals[i].x - (parallax.originals[i].x / timeOut) * delta);

						// Set style to caption element
						captionEl.style['marginLeft'] = x + 'px';
					}

					// Calculate Y parallax dimension to original value
					if (parallaxDimensions.y) {
						var y = (parallax.originals[i].y - (parallax.originals[i].y / timeOut) * delta);
					...
					// Calculate 3D parallax dimensions
					if (parallaxDimensions.z && gpuAcceleration) {
						var rX = parallax.originals[i].rX - ((parallax.originals[i].rX / timeOut) * delta),
		...
		self.destroy = function () {
			...
			if ($slides[0]) {
				$slides.removeAttr('style').removeClass(minnamespace + 'Slide').eq(self.relative.activeSlide).removeClass(self.options.classes.activeClass);
			...
			if (!self.options.buttons.forward && $forwardButton[0]) {
				removeEls.push($forwardButton[0]);
			}
			if (!self.options.buttons.backward && $backwardButton[0]) {
				removeEls.push($backwardButton[0]);
			}
			if (!self.options.buttons.prev && $prevButton[0]) {
				removeEls.push($prevButton[0]);
			}
			if (!self.options.buttons.next && $nextButton[0]) {
				removeEls.push($nextButton[0]);
			}
			if (!self.options.buttons.prevPage && $prevPageButton[0]) {
				removeEls.push($prevPageButton[0]);
			}
			if (!self.options.buttons.nextPage && $nextPageButton[0]) {
				removeEls.push($nextPageButton[0]);
			}
			if (!self.options.buttons.fullScreen && $fullScreenButton[0]) {
				removeEls.push($fullScreenButton[0]);
			}

			if (!self.options.pages.pagesBar && $pagesBar[0]) {
			...
			if (!self.options.thumbnails.thumbnailsBar && $thumbnailsBar[0]) {
			...
			$slideElement.css(transform || (self.options.navigation.horizontal ? 'left' : 'top'), transform ? 'none' : 0).removeClass(minnamespace + 'SlideElement');
			// Remove the instance from element data storage
			$frame.removeData(namespace).removeClass(minnamespace + 'Frame');
		...
		self.init = function () {
			...
			$slides.css({ 'position': 'relative' }).addClass(minnamespace + 'Slide');
	...
	function getCaptionStyles($caption) {
		...
		for (var i = 0, len = captionResponsiveStyles.length; i < len; i++) {
			var property = captionResponsiveStyles[i],
	...
	function parseVideoURL(url) {
		...
		for (var i = 0, len = videoRegularExpressions.length; i < len; i++) {
			var object = videoRegularExpressions[i];
	...
	function parsePhotoURL(url) {
		...
		for (var i = 0, len = photoRegularExpressions.length; i < len; i++) {
			var object = photoRegularExpressions[i];
	...
	*/
	...
	mightySlider.defaults = {
		...
		// Parallax
		parallax: {
			x:       1, // Enable automatic cycling by 'slides' or 'pages'.
			y:       1, // Delay between cycles in milliseconds.
			z:       0, // Delay between cycles in milliseconds.
			invertX:       1, // Delay between cycles in milliseconds.
			invertY:       1, // Delay between cycles in milliseconds.
			revert:  1,    // Whether the layers should revert to its start position when parallax stops.

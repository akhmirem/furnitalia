...
function furnimobile_preprocess_page(&$vars) {
	...
	if (variable_get('show_banner', FALSE)) {
		if (drupal_match_path(current_path(), implode("\n", array_merge(array('node/*', 'collections', 'taxonomy/term/*'), $italia_editions_gallery_paths)))) {
			...
			$banner_html = l('<img src="' . $banner_img_path . '"/>', "living", array('html' => TRUE, 'attributes' => array('class' => array('banner-link')), 'query' => array('availability' => 'in_stock'))); //'query' => array('utm_source' => 'mobile', 'utm_medium' => 'banner', 'utm_campaign' => 'moving-sale'

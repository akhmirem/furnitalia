...
function furnimobile_preprocess_page(&$vars) {
	...
	if (variable_get('show_banner', FALSE)) {
		if (drupal_match_path(current_path(), implode("\n", array_merge(array('node/*', 'collections', 'taxonomy/term/*'), $italia_editions_gallery_paths)))) {
			...
			$banner_img_path = $files_dir . "/promo/moving_sale_march/320x140_MobileSecondary.jpg";
			$banner_html = l('<img src="' . $banner_img_path . '"/>', "moving-sale", array('html' => TRUE, 'query' => array('utm_source' => 'mobile', 'utm_medium' => 'banner', 'utm_campaign' => 'moving-sale'), 'attributes' => array('class' => array('banner-link'))));
...
function furnimobile_form_views_exposed_form_alter(&$form, &$form_state, $form_id) {  
	...
	//drupal_add_css($base_path . $theme_path . "/lib/dropkick/dropkick.css");
